...
- `Receiver`: a baseband receiver implementation. Requires a buffer attached with the `Receiver.set_buffer()` method. It processes the samples contained in the buffer to detect and decode potential LoRa frames, according to provided frame parameters (`bw`, `sf`, `cr`).
...
- A continuous mode (default), where it continuously look for and decode incoming frames, report decoded frame through a registered callback, wait for more samples if empty-buffer is reached, and start over. This is the recommended mode for interfacing with SDR.
...
## Dependencies:
The library itself only depends on `numpy`. Depending on your use case, additional libraries might be required:
